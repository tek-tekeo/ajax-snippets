  ...
  <div>
    ...
    <v-row>
      <v-col cols="4">
        <wp-text-area
          label="IOSのアフィリンク先"
          v-model="app.iosAffiLink"
        ></wp-text-area>   
      </v-col>
      <v-col cols="4">
        <wp-text-area
          label="Androidのアフィリンク先"
          v-model="app.androidAffiLink"
        ></wp-text-area>   
      </v-col>
      <v-col cols="4">
        <wp-text-area
    ...
    <v-row>
      <v-col cols="4">
        <wp-text-box
          label="アプリの表示順"
          v-model="app.appOrder"
        ></wp-text-box>   
      </v-col>
      <v-col cols="4">
        <wp-text-box
          label="アプリの料金"
          v-model="app.appPrice"
        ></wp-text-box>   
      </v-col>
    </v-row>
    <v-row>
...
module.exports = {
  ...
  methods:{
    updateApp(){
      this.$emit("updated-item", this.app);
    },
    analizeAffi(){
      if(this.appCode != ''){
      const appLinks = new RegExp('(https://[^"]*)','g');
        const appRes = this.appCode.match(appLinks);
        this.$set(this.app, 'img', appRes[0]);
        this.$set(this.app, 'iosLink', appRes[2]);
        this.$set(this.app, 'androidLink', appRes[4]);
        const appDev = new RegExp('<span class=\"appreach__developper\">([^<]+)</span>');
        this.$set(this.app, 'dev', this.appCode.match(appDev)[1]);
